...
\begin{figure}[htpb]
	...
	\caption{Pictured is a theoretical network scenario for linearly placed single-line households. The simplest coordination possible is depicted here, with household pairs $\{1,\,2\}$ and $\{3,\,4\}$ in coordination. The pattern is repeated up to $n$ households, whereby $n$ is either odd or even.}
	\label{comm_topology}
\end{figure}
\FloatBarrier
...
The theoretical radial network demonstrates a relatively straightforward load pattern. Every consecutively added household down the line increases the overall network load; the last household demonstrates the network minimum in voltage magnitude.

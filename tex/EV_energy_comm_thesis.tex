...
Referring back to \cref{comm_topology}, the energy community decides to allocate charging times iteratively, starting from the household closest to the substation. The first household will take the charging time closest to peak grid hours in the evening, the second will take the next available time slot after the first and so on.



This remark is also an approximation however, and only applies for sufficiently large $n$. For example, in case of $n$ being an odd number of households, the remainder household will simply be treated as in pair to itself, or in other words, uncoordinating.



\subsubsection{Test feeder application}
Line loading is generally expected to be around 1\% per household line (and was shown not to be the limiting factor for hosting capacity calculation).\\
on avg there will be 55 sgens / 12 hours = 4.583 sgens per given hour in the night frame
...
Energy community function doesn't start at the peak hours perfectly because jezzus christe, that's a lot of code to rewrite for no real benefit
